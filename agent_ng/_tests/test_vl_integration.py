...
TEST_IMAGE = Path(os.getenv("VL_TEST_IMAGE", str(_TEST_FILES / "test_image.jpg")))
TEST_VIDEO = Path(os.getenv("VL_TEST_VIDEO", str(_TEST_FILES / "test_video.mp4")))
TEST_AUDIO = Path(os.getenv("VL_TEST_AUDIO", str(_TEST_FILES / "test_audio.mp3")))
...
class TestVLCapabilityFlags:
    ...
    def test_qwen3_6_plus_has_image_video(self):
        ...
        assert model.get("vision_support") is True, (
            "Qwen 3.6 Plus should support images"
        )
    ...
    def test_gemini_2_5_flash_has_all_modalities(self):
        ...
        gemini_models = [
            m for m in self._all_models() if "gemini-2.5-flash" in m["model"]
        ]
    ...
    def test_mimo_v2_5_is_text_only_via_openrouter(self):
        model = next(
            (m for m in self._all_models() if m["model"] == "xiaomi/mimo-v2.5"), None
        )
    ...
    def test_mimo_v2_omni_is_omni_on_openrouter(self):
        model = next(
            (m for m in self._all_models() if m["model"] == "xiaomi/mimo-v2-omni"), None
        )
    ...
    def test_claude_has_image_only(self):
        claude_models = [
            m for m in self._all_models() if "claude-sonnet-4" in m["model"]
        ]
        assert len(claude_models) >= 1
        for m in claude_models:
            assert m.get("vision_support") is True, (
                f"{m['model']} should support images"
            )
    ...
    def test_grok_is_text_only_on_openrouter(self):
        ...
        for m in grok_models:
            assert not m.get("vision_support"), (
                f"{m['model']} should be text-only on OpenRouter"
            )
...
class TestVisionToolManagerRouting:
    ...
    def test_adapter_openrouter_for_qwen(self):
        ...
        assert adapter is mgr.adapters.get("openrouter"), (
            "Qwen should use OpenRouter adapter"
        )
    ...
    def test_adapter_gemini_direct_for_bare_gemini(self):
        ...
        assert adapter is mgr.adapters.get("gemini"), (
            "gemini-2.5-flash should use Gemini Direct adapter"
        )
    ...
    def test_adapter_openrouter_for_google_prefixed(self):
        ...
        assert adapter is mgr.adapters.get("openrouter"), (
            "google/ prefix → OpenRouter adapter"
        )
...
class TestImageAnalysisAPI:
    ...
    @pytest.mark.skipif(
        not TEST_IMAGE.exists(), reason=f"Test image not found: {TEST_IMAGE}"
    )
    def test_analyze_image_with_qwen(self):
        ...
        assert isinstance(result, str) and len(result) > 20, (
            f"Expected description, got: {result!r}"
        )
        print(f"\n[IMAGE] Qwen response:\n{result[:300]}")
    ...
    def test_analyze_image_ai_tool(self):
        ...
        raw = analyze_image_ai.func(
            filename=str(TEST_IMAGE), prompt="What shapes do you see?", agent=None
        )
        data = _result(raw)
...
class TestVideoAnalysisAPI:
    ...
    @pytest.mark.skipif(
        not TEST_VIDEO.exists(), reason=f"Test video not found: {TEST_VIDEO}"
    )
...
class TestAudioAnalysisAPI:
    ...
    def test_analyze_audio_with_gemini(self):
        ...
        result = mgr.analyze_audio(
            ...
            prompt="Transcribe or describe what you hear in this audio.",
        )
        assert isinstance(result, str) and len(result) > 10, (
            f"Expected transcription, got: {result!r}"
        )
    ...
    def test_understand_audio_tool(self):
        ...
        raw = understand_audio.func(
            filename=str(TEST_AUDIO), prompt="What do you hear?", agent=None
        )
        data = _result(raw)
        assert data.get("result"), f"Expected result, got: {data}"
        print(f"\n[AUDIO TOOL] Response:\n{data['result'][:300]}")
