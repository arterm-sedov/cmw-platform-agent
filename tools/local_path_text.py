...
def read_local_path_to_plain_text(
    ...
    *,
    ...
    Returns:
        (text, error, encoding, image_paths, markdown_path)
    """
    file_info = _file_info if _file_info is not None else FileUtils.get_file_info(file_path)
    if not file_info.exists:
        return "", file_info.error or "File not found", None, [], None
    ...
    if ext in (".md", ".markdown"):
        ...
        if not result.success:
            return "", result.error or "Markdown read failed", None, [], None
    ...
    if ext in (".png", ".jpg", ".jpeg", ".gif", ".webp", ".bmp", ".tiff", ".tif"):
        ...
        try:
            ...
            if t:
                return str(result.text_content), None, None, [], None
            return (
        ...
        try:
            ...
            if extract_images:
                ...
                if not asset_result.success:
                    return "", asset_result.error_message or "PDF extraction failed", None, [], None
                content = asset_result.markdown_content or asset_result.text_content
                image_paths = asset_result.image_paths
                markdown_path = asset_result.markdown_path
            else:
                pdf_result = PDFUtils.extract_text_from_pdf(file_path, use_markdown=True)
                if not pdf_result.success:
                    return "", pdf_result.error_message or "PDF extraction failed", None, [], None
        ...
        try:
            if extract_images:
                ...
                if not content or not str(content).strip():
                    return "", f"No text content found in {ext} file", None, [], None
                image_paths = []
                markdown_path = None
            if ext == ".html" and not read_html_as_markdown:
                ...
                if raw.success and raw.content is not None:
                    return raw.content, None, raw.encoding, [], None
                return "", raw.error or "Could not read HTML as raw text", None, [], None
            return str(content), None, None, image_paths, markdown_path
        except Exception as e:
            return "", f"Error processing {ext}: {e!s}", None, [], None
