...
## Process model template vs record template scope (`RecordType` / `doc.*`)
...
| `doc.{N}` | **Process model template** (`#RecordType/doc.{N}/…`); API owner type **`DocumentationTemplate`** — internal name only, not user-facing prose | e.g. `doc.1`–`doc.69` on FM hosts — enumerate all `doc.*`, not only `doc.1` |
...
**Rule:** `doc.*`-scoped work (Operations, Lists, Forms, Context on a **process model template**) belongs with the **`doc.*`** id in the instance playbook or progress JSON `meta.template` / `process_model_templates[]` — not mixed into unrelated `oa.*` record-template batches unless the batch explicitly spans both.
