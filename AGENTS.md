...
- **Programmatic API surface:** Footer **API docs** intentionally expose only **`/ask`** (from **`gr.api`**) and **`/ask_stream`** (hidden **`Textbox.submit`** — see **`agent_ng/app_ng_modular.py`**). **`Blocks`** UI listeners use **`api_visibility="private"`** (explicitly or via **`apply_concurrency_to_*`** defaults in **`agent_ng/queue_manager.py`**) so the OpenAPI explorer stays small and fast.
