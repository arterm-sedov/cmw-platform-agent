...
- **Programmatic API surface:** **`Blocks`** listeners default to **`api_visibility="private"`** (explicit passes in tabs / **`demo.load`** / **`gr.Timer.tick`**, or **`apply_concurrency_to_*`** in **`agent_ng/queue_manager.py`**). Only **`ask`** (**`gr.api`**) and **`ask_stream`** (hidden **`Textbox.submit`** in **`agent_ng/app_ng_modular.py`**) stay **`public`**, so **`get_api_info()`** / **`openapi.json`** only enumerate those endpoints. The in-app footer **API** view can still hitch on huge UIs (client SPA work); **`CMW_UI_GRADIO_HIDE_API_FOOTER=1`** drops the footer link (keep using **`GET /gradio_api/info`**, **`/gradio_api/openapi.json`**, or **`gradio_client`**). On startup we background-prime **`/gradio_api/info`** because **`demo.launch(debug=True)`** blocks the main thread and would otherwise defer that work to the first click.
- **Operational UI env:** Shell or local **`.env`** only (not **`.env.example`**); contracts in **`agent_ng/agent_config.py`**. **`CMW_UI_DOWNLOAD_PREP_AFTER_STREAM`** — also refresh downloads on streaming completion (default off). **`CMW_UI_GRADIO_HIDE_API_FOOTER`** — hide the footer **API** shortcut (footer still defaults to **`["api"]`**). Markdown/HTML export prep always generates **HTML** alongside Markdown when files are built. **Layout:** **`gr.Sidebar`** (native collapsible panel — upstream **`gradio/layouts/sidebar`**) wraps quick actions + progress + token budget; **LLM selection** mounts in the **Config** tab. **`CMW_USE_DOTENV`** only switches **Comindware platform** credentials (**`CMW_BASE_URL`**, **`CMW_LOGIN`**, **`CMW_PASSWORD`** from **`.env`** vs URL/username/password in the Config UI); it does not hide the Config tab. **Statistics** tab uses a single **`stats_display`** (`elem_id=stats-display`); **`update_status`**, **`update_all_ui`**, the stats timer, chat end-of-turn chains, and model-switch **`.then(refresh_stats)`** all refresh that block (**`format_stats_display`**). Session-scoped dropdown sync still uses **`demo.load`** (**`Request`** session id). **`get_demo_with_language_detection()`** rebuilds cached **`demo`** when **`CMW_UI_DOWNLOAD_PREP_AFTER_STREAM`** changes (**`_ui_layout_env_signature`**); restart the app after other env changes.
