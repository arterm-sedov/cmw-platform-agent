...
- **Programmatic API surface:** Footer **API docs** expose no public Gradio endpoints — all are `api_visibility="private"`. The sole public API is `POST /api/v1/chat/completions` (native FastAPI route with bearer auth, real SSE streaming). **`Blocks`** UI listeners use **`api_visibility="private"`** (explicitly or via **`apply_concurrency_to_*`** defaults in **`agent_ng/queue_manager.py`**) so the OpenAPI explorer stays small and fast.
